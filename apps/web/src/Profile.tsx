...
type Summary = {
  since: string
  totals: { solved: number; played: number; gaveUp: number; guesses: number; characters: number }
  ...
  streak: { current: number; best: number; week: { day: string; played: boolean }[] }
  games: { game: GameId; pool: number; solved: number }[]
  modes: { mode: string; played: number; won: number; guesses: number }[]
  recent: { game: string; mode: string; answerId: number; status: string; guessCount?: number; daily?: string; finishedAt?: string; name: Named | null }[]
}
...
export default function Profile({ onBack }: { onBack: () => void }) {
  ...
  const date = (value?: string, long = false) => {
    ...
    if (long) return when.toLocaleDateString(LOCALES[lang], { month: 'long', year: 'numeric' })
          ...
          <section className="card profile-card">
            ...
            <div className="profile-facts">
              ...
              <div>
                ...
                <b className="small">{summary?.favourite ? l(gameLabel(summary.favourite)!) : '—'}</b>
              </div>
              <div>
            ...
            {summary?.games.length ? (
              <ul className="worlds">
                {summary.games.map((row) => {
                  ...
                  return (
                    <li key={row.game}>
                      <span className="world-tag" style={{ background: game?.accent ?? 'var(--accent)' }} aria-hidden>
                      ...
                      <span className="world-bar">
                        <i style={{ width: `${Math.max(percent(row.solved, row.pool), 2)}%`, background: game?.accent ?? 'var(--accent)' }} />
                      </span>
                    </li>
                  )
                })}
                ...
                {summary.modes.map((row) => (
                  <li key={row.mode}>
                    ...
                    <span className="mode-bar">
                      <i style={{ width: `${Math.max(percent(row.won, row.played), 2)}%` }} />
                    </span>
                  </li>
                ))}
              </ul>
                ...
                {summary.recent.map((round, index) => (
                  <li key={`${round.game}-${round.answerId}-${index}`}>
                    ...
                    <span className="activity-text">
                      ...
                      <small>
                        {gameLabel(round.game) ? l(gameLabel(round.game)!) : round.game} ·{' '}
