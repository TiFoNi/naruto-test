...
export default function Profile({ onBack }: { onBack: () => void }) {
  ...
  const saveNickname = async (event: FormEvent) => {
    ...
    setNickMessage(
      code
        ? { ok: false, text: errorText(code) }
        : { ok: true, text: t("profile.saved") },
    );
  };
  ...
  const date = (value?: string, long = false) => {
    ...
    if (long)
      return when.toLocaleDateString(LOCALES[lang], {
        month: "long",
        year: "numeric",
      });
    const today = new Date().toDateString() === when.toDateString();
    return today
      ? when.toLocaleTimeString(LOCALES[lang], {
          hour: "2-digit",
          minute: "2-digit",
        })
      : when.toLocaleDateString(LOCALES[lang], {
          day: "numeric",
          month: "short",
        });
  };
  ...
  const modeLabel = (id: string) =>
    MODES.find((m) => m.id === id)?.label ?? "mode.classic";
  const accuracy = summary
    ? percent(summary.totals.solved, summary.totals.played)
    : 0;
  const average =
    summary && summary.totals.solved
      ? (summary.totals.guesses / summary.totals.solved).toFixed(1)
      : "—";
        ...
        <div className="profile-column">
          <section className="card profile-card">
            ...
            <div className="profile-facts">
              <div>
                ...
                <b>{summary?.rank ? `#${summary.rank.position}` : "—"}</b>
                {summary?.rank && (
                  <small>{`${lang === "en" ? "of" : lang === "uk" ? "з" : "из"} ${summary.rank.players}`}</small>
                )}
              </div>
              <div>
              ...
              <div>
                ...
                <b className="small">
                  {summary?.favourite ? l(gameLabel(summary.favourite)!) : "—"}
                </b>
              </div>
              <div>
                <span>{t("profile.since")}</span>
                <b className="small">{date(summary?.since, true)}</b>
              </div>
            </div>
                ...
                <form onSubmit={saveNickname}>
                  ...
                  <div className="inline-field">
                    ...
                    <button
                      className="primary"
                      type="submit"
                      disabled={savingNick || nickname.trim() === user.nickname}
                    >
                  ...
                  {nickMessage && (
                    <div
                      className={nickMessage.ok ? "notice ok" : "notice error"}
                    >
                      {nickMessage.text}
                    </div>
                  )}
          ...
          <section className="card streak-card">
            <header>
              ...
              <span className="muted">
                {t("profile.streakRecord", { days: summary?.streak.best ?? 0 })}
              </span>
            </header>
            <div className="streak-now">
              ...
              <div>
                ...
                <small
                  className={
                    summary?.streak.week.at(-1)?.played ? "ok" : "muted"
                  }
                >
                  {summary?.streak.week.at(-1)?.played
                    ? t("profile.streakToday")
                    : t("profile.streakIdle")}
                </small>
              </div>
            </div>
            <ul className="streak-week">
              {(summary?.streak.week ?? []).map((day, index) => (
                <li key={day.day} className={day.played ? "on" : ""}>
                  ...
                  <small>
                    {WEEKDAYS[lang][
                      (new Date(`${day.day}T00:00:00Z`).getUTCDay() + 6) % 7
                    ] ?? index}
                  </small>
                </li>
              ))}
          ...
          <section className="card soon-card">
            <header>
              ...
              <span className="soon-badge">{t("profile.soon")}</span>
            </header>
          ...
          <section className="profile-metrics">
            ...
            <div className="card">
              ...
              {!!summary?.duels.played && (
                <small>
                  {percent(summary.duels.wins, summary.duels.played)}% ·{" "}
                  {summary.duels.wins}–{summary.duels.losses}
                </small>
              )}
            </div>
            <div className="card">
                      ...
                      return (
                        <li key={row.game}>
                          <span
                            ...
                            aria-hidden
                          >
                            {l(game?.label ?? { ru: "", uk: "", en: "" }).slice(
                              0,
                              2,
                            )}
                          </span>
                          <span className="world-name">
                            {game ? l(game.label) : row.game}
                          </span>
                          <span className="world-count">
                            <b>{row.solved}</b> / {row.pool} ·{" "}
                            {percent(row.solved, row.pool)}%
                          </span>
                  ...
                  <ul className="activity">
                    {summary.recent.map((round, index) => (
                      <li key={`${round.game}-${round.answerId}-${index}`}>
                        <span
                          className={`activity-dot ${round.status}`}
                          aria-hidden
                        />
                        <span className="activity-text">
                          ...
                          <small>
                            {gameLabel(round.game)
                              ? l(gameLabel(round.game)!)
                              : round.game}{" "}
                            · {t(modeLabel(round.mode))}
                            {round.guessCount
